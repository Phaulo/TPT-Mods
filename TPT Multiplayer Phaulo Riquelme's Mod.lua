...
local annoying_member_alert_timer = 0
local annoying_member_alert_text = ""
local annoying_member_name = "Canada_Ball"
local annoying_member_x = 0
local annoying_member_y = 0
local annoying_member_timer = 0
local annoying_member_detected = false
local annoying_members = {}
...
local function is_not_smart_user(username)
	return not smart_users[username]
end

local function is_smart_user(username)
	return smart_users[username]
end

local function warn_about_annoying_member(text,client_i)
    if text == nil then
        text = "There's a Annoying Member"
    end
    if types[type(text)] == nil then
        text = "There's a Annoying Member"
	end
    annoying_member_alert_timer = 10
    if is_smart_user(tpt.get_name()) then
        annoying_member_alert_text = tostring(text)
    else
        annoying_member_alert_text = ""
    end
    annoying_member_timer = 500
    annoying_member_detected = true
    if is_smart_user(tpt.get_name()) then
        if client_i then
            client_i.window_:backlog_push_say("[System]", "Annoying Member Warning: "..tostring(text))
        end
    end
end
...
require_preload__["tptmp.client"] = function()
	...
	elseif tpt.version.mobilemajor and false then
	...
	local function run()
		...
		local function handle_tick()
			...
			if cli then
                tpt.members = cli.id_to_member
				for _, member in pairs(tpt.members) do
					if member:can_render() then
						...
						local color = member.nick_colour_seed_
						if color ~= 0 then
                            local pcur_r, pcur_g, pcur_b, _ = gfx.getColors(color)
                        end
                        if annoying_member_name == member.nick then
                            annoying_member_x = px
                            annoying_member_y = py
                        end
						if rx then
							sx, sy = 0, 0
						end
                ...
                tpt.members = {}
annoying_member_alert_timer = 0
annoying_member_alert_text = ""
annoying_member_name = "Canada_Ball"
annoying_member_x = 0
annoying_member_y = 0
annoying_member_timer = 0
annoying_member_detected = false
annoying_members = {}
			end
...
require_preload__["tptmp.client.client"] = function()
	...
	function client_i:handle_sync_30_()
		...
		if ok then
            ...
            if is_not_smart_user(member.nick) then
annoying_members[member.nick] = 10
                annoying_member_name = member.nick
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                warn_about_annoying_member(member.formatted_nick.." Synced",self)
            end
		else
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                warn_about_annoying_member(member.formatted_nick.." Stamped",self)
            end
		else
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(desc.format:format(desc.states[value + 1], member.formatted_nick),self)
                end
			end
		end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(("Ambient air temperature set to %.2f by %s"):format(set, member.formatted_nick),self)
                end
		end
	...
	function client_i:handle_flood_39_()
		...
		util.flood_any(x, y, member.last_tool, -1, -1, member)
            warn_about_annoying_member(member.formatted_nick.." Is Drawing",self)
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." made a Single-frame step",self)
                end
	end
	...
	function client_i:handle_sparkclear_60_()
		...
		self.log_event_func_(colours.commonstr.event .. "Sparks cleared by " .. member.formatted_nick)
                if is_not_smart_user(member.formatted_nick) then
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." Cleared Sparks",self)
                end
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." Cleared Pressure",self)
                end
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." Inverted the Pressure",self)
                end
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." Cleared Simulation",self)
                end
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." reseted Ambient heat",self)
                end
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." Loaded Online save " .. (hist == 0 and "id" or "history") .. ":" .. id,self)
                end
		end
	end
...
annoying_members[member.nick] = 10
                ...
                annoying_member_x = member.pos_x
                annoying_member_y = member.pos_y
                    warn_about_annoying_member(member.formatted_nick.." Reloaded Simulation",self)
                end
	end
	...
	function client_i:reformat_nicks_()
		...
		for _, member in pairs(self.id_to_member) do
			...
			member.nick_colour_seed_ = self.nick_colour_seed_
		end
	end
...
require_preload__["tptmp.client.colours"] = function()
	...
	local appearance = {
		hover = {
			background = tpt.version.mobilemajor and {   0,   0,   0 } or {  20,  20,  20 },
			text       = { 255, 255, 255 },
			border     = { 255, 255, 255 },
		},
		inactive = {
...
require_preload__["tptmp.client.profile.vanilla"] = function()
	...
	function profile_i:handle_mousedown(px, py, button)
		...
		if not self.placing_zoom_ then
			...
			if px < sim.XRES and py < sim.YRES then
				...
				self:update_draw_mode_()
				self.dragging_mouse_ = true
				if is_not_smart_user(tpt.get_name()) then
                    warn_about_annoying_member("You're Drawing, Maybe There's One You're Trying to Annoy Him.")
                end
	...
	function profile_i:handle_keypress(key, scan, rep, shift, ctrl, alt)
		...
		if scan == sdl.SDL_SCANCODE_SPACE then
				if is_not_smart_user(tpt.get_name()) then
                    annoying_member_name = tpt.get_name()
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                    warn_about_annoying_member("You Paused the Simulation, Maybe There's One You're Trying to Annoy Him.")
                end
			self.simstate_invalid_ = true
		...
		elseif scan == sdl.SDL_SCANCODE_F5 or (ctrl and scan == sdl.SDL_SCANCODE_R) then
            if is_not_smart_user(tpt.get_name()) then
                    ...
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                warn_about_annoying_member("You Reloaded, Maybe There's One You're Trying to Annoy Him.")
            end
			self:button_reload_()
		elseif scan == sdl.SDL_SCANCODE_F and not ctrl then
			if ren.debugHUD() == 1 and (shift or alt) then
				...
				if is_not_smart_user(tpt.get_name()) then
                    ...
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                    warn_about_annoying_member("You're Trying to Make a Framestep, Maybe There's One You're Trying to Annoy Him.")
                end
		...
		elseif scan == sdl.SDL_SCANCODE_U then
			if ctrl then
				...
				if is_not_smart_user(tpt.get_name()) then
                    ...
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                    warn_about_annoying_member("You Reseted the Ambient Air Temperature, Maybe There's One You're Trying to Annoy Him.")
                end
			else
				self.simstate_invalid_ = true
			end
		...
		elseif scan == sdl.SDL_SCANCODE_EQUALS then
			if ctrl then
				...
				if is_not_smart_user(tpt.get_name()) then
                    ...
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                    warn_about_annoying_member("You Reseted the Spark, Maybe There's One You're Trying to Annoy Him.")
                end
			else
				...
				if is_not_smart_user(tpt.get_name()) then
                    ...
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                    warn_about_annoying_member("You Reseted the Pressure, Maybe There's One You're Trying to Annoy Him.")
                end
			end
		...
		elseif scan == sdl.SDL_SCANCODE_I and not ctrl then
			self:report_airinvert_()
				if is_not_smart_user(tpt.get_name()) then
                    ...
                    annoying_member_x = tpt.mousex
                    annoying_member_y = tpt.mousey
                    warn_about_annoying_member("You Inverted the Pressure, Maybe There's One You're Trying to Annoy Him.")
                end
...
require_preload__["tptmp.client.side_button"] = function()
  ...
  if inside then
   if self.text_timer < 100 then
    self.text_timer = self.text_timer+4
   else
    self.text_timer = 100
   end
  else
   if self.text_timer > 0 then
    self.text_timer = self.text_timer-2
   else
    self.text_timer = 0
   end
  end
		...
		local info_text = "Multiplayer"
		gfx.drawText(self.pos_x_-tpt.textwidth(info_text)-8, self.pos_y_+((self.height_-8)/2), info_text, unpack({255,255,255,self.text_timer*2.55}))
	end
	...
	local function new(params)
		...
		if tpt.version.mobilemajor then
			pos_y = pos_y + 27
			width = 29
			height = 24
		end
...
require_preload__["tptmp.client.window"] = function()
   ...
   if cli() then
	...
	function window_i:backlog_push_(collect, important)
	 if not self.backlog_unique_ then
      self.backlog_unique_ = 0
  end
		self.backlog_unique_ = self.backlog_unique_ + 1
		if not self.backlog_last_ then
      self.backlog_last_ = {prev = {next = {}}}
  end
		local msg = {
	...
	function window_i:handle_mousedown(px, py, button)
		...
		if util.inside_rect(self.pos_x_, self.pos_y_+self.height_-30, self.width_, 30, util.mouse_pos()) and tpt.version.mobilemajor then
--print(self.client_func_)
            local str = tpt.input("Input")
            if str ~= nil then
                self.input_text_1_ = str
                if #str > 0 then
                    if string.get(str,1) == "/" then
                        if self.localcmd then
                            self.localcmd:parse(str)
                        end
                    else
                        local cli = self.client_func_
                        if cli ~= nil then
                            cli():send_say(colours.commonstr.chat .. "<" .. cli():formatted_nick() .. colours.commonstr.chat .. "> " .. str)
                            pcall(function()
                                self:backlog_push_say_other(cli():formatted_nick(),str)
                            end)
                        else
                            tpt.log("Error Saying")
                        end
                    end
                end
            end
...
tpt.register_step(function()
    annoying_member_alert_timer = annoying_member_alert_timer-(10/255)
    annoying_member_timer = annoying_member_timer-1
    if annoying_member_detected and annoying_member_timer <= 0 then
        annoying_member_detected = false
    end
    gfx.drawText((sim.XRES/2)-(tpt.textwidth(annoying_member_alert_text)/2), sim.YRES/2, annoying_member_alert_text, 255, 255/4, 255/4, annoying_member_alert_timer*255)
    local count = 0
    for _, member in pairs(tpt.members) do
        local name = member.nick
        if annoying_members[name] then
            count = count+1
            annoying_members[name] = annoying_members[name]-(10/255)
            if annoying_members[name] <= 0 then
                annoying_members[name] = nil
            end
        end
    end
if #tpt.members > 0 then
    gfx.drawText(5, sim.YRES-12-2, "Annoying Members: "..count, 255, 255/8, 255/8, 255)
end
if annoying_member_name == tpt.get_name() then
    annoying_member_x = tpt.mousex
    annoying_member_y = tpt.mousey
end
_G.annoying_member_alert_timer = annoying_member_alert_timer
_G.annoying_member_alert_text = annoying_member_alert_text
_G.annoying_member_name = annoying_member_name
_G.annoying_member_x = annoying_member_x
_G.annoying_member_y = annoying_member_y
_G.annoying_member_timer = annoying_member_timer
_G.annoying_member_detected = annoying_member_detected
_G.annoying_members = annoying_members
end)

if tpt.version.jacob1s_mod or tpt.version.mobilemajor then -- Jacob1's Mod and Mobile Version Only
...
local function ESwindowUpdate()
    for stkm = 3,102,1 do
        ...
        if tpt.set_pause() == 0 and sim.stickman(stkm,"spwn") >= 1 then
            ...
            if annoying_member_detected then
                local facing = 0
